...
public class Person {
    ...
    public boolean isValidBirthDate(LocalDate bdate) {
        ...
        if (bdate.isAfter(LocalDate.now())) {
            logger.info("Birth date must not be in future.");
            return false;
        }
        return true;
    }
    ...
    public boolean isValidPerson() {
        ...
        if (firstName == null || firstName.trim().isEmpty()) {
            logger.info("First name must contain minimum one character.");
            isValid = false;
        }
        if (lastName == null || lastName.trim().isEmpty()) {
            logger.info("Last name must contain minimum one character.");
            isValid = false;
        }
        if (!isValidBirthDate(birthDate)) {
            isValid = false;
